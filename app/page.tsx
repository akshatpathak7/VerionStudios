...
function Brand() { return <a className="brand" href="#top" aria-label={`${studio.name} home`}><img src={studio.logo} width="36" height="36" alt=""/><span>VERION<span className="brand-sub">STUDIOS</span></span></a>; }
export default function Home() {
 ...
 return <div id="top" className={paused?"site motion-paused":"site"}>
  ...
  <main id="main">
   ...
   <section id="contact" className="contact wrap section"><div className="eyebrow">03 / LET’S MAKE SOMETHING GOOD</div><div className="contact-row"><h2>Your next idea.<br/><span>Our next obsession.</span></h2><a className="contact-arrow" href={studio.email?`mailto:${studio.email}`:"#contact-details"} aria-label={studio.email?"Email Verion Studios":"View contact details"}><ArrowUpRight strokeWidth={1}/></a></div><div className="contact-details" id="contact-details"><p>Have footage to transform or something to build?<br/>Let’s bring it to life.</p>{studio.email?<a className="email-link" href={`mailto:${studio.email}`}>{studio.email} <ArrowUpRight size={18}/></a>:<span className="contact-pending">Contact details coming soon</span>}</div></section>
  </main>
